...
variables = df[['v2500',                            # Education
                ...
                'v2534',                            # Physical assault on inatme violations
                'v2535',                            # COUNT Physical assault on inatme violations
...
# physical assault inmate violation count
df_statsbinary['v2534'] = df_statsbinary['v2534'].apply(lambda v: 1 if v == 'Yes' else 0)
df_statsbinary['v2535'] = np.where(
    df_statsbinary['v2534'] == 0,  
    0,                       
    df_statsbinary['v2535']      
)
print(df_statsbinary['v2535'].value_counts())
df_statsbinary['v2535'] = df_statsbinary['v2535'].apply(lambda v: int(v) if not \
                                            (v == 'Blank' or
                                            v == 'Don\'t know' or
                                            v == 'Refused')
                                            else None)
df_statsbinary = df_statsbinary.dropna(subset=['v2535'])
...
NOTE: Dropped all of the facilities with less than 40 people, as that represented about the bottom 15% of facilities, which otherwise cause perfect prediction
'''
'''
...
df_statsbinary['v2984'] = np.where(
    df['v2984'].value_counts() <= 40,
    None,                       # TODO Jul 3rd -- Currently working on trying to fix this -- I want to get rid of facilities with less than 40 people
    df_statsbinary['v2535']      
)
df_statsbinary = df_statsbinary.dropna(subset=['v2984'])
facility_dummies = pd.get_dummies(df_statsbinary['v2984'], prefix='control_facilityID', drop_first=False, dtype=int) 
...
facilities_less_than_40 = [130, 263, 299, 250, 282, 216, 375, 261, 249, 275, 363, 269, 238, 351, 141, 365, 221, 126, 217, 219, 296, 116, 180, 356, 215, 378, 167, 174, 326, 302, 150, 232, 125, 123, 260, 361, 295]

for ID in facilities_less_than_40:
    print(f"dropped control_facilityID_{ID}")
    df_statsbinary = df_statsbinary.drop(f'control_facilityID_{ID}', axis=1)
...
output = df_statsbinary['v2535']
inputs = df_statsbinary.drop(['v2534', 'v2535', 'v0717'], axis = 1)    

# Show odds values as well
pd.set_option('display.max_rows', None)    # Show all rows
log_reg = sm.NegativeBinomial(output, inputs).fit()
