...
variables = df[['v2500',                            # Education
                ...
                'v2522',                            # weapon violations
                'v2523',                            # count weapon violations
...
# weapon rule violation count
df_statsbinary['v2522'] = df_statsbinary['v2522'].apply(lambda v: 1 if v == 'Yes' else 0)
df_statsbinary['v2523'] = np.where(
    df_statsbinary['v2522'] == 0,  
    0,                       
    df_statsbinary['v2523']      
)
print(df_statsbinary['v2523'].value_counts())
df_statsbinary['v2523'] = df_statsbinary['v2523'].apply(lambda v: int(v) if not \
                                            (v == 'Blank' or
                                            v == 'Don\'t know' or
                                            v == 'Refused')
                                            else None)
df_statsbinary = df_statsbinary.dropna(subset=['v2523'])
...
output = df_statsbinary['v2523']
inputs = df_statsbinary.drop(['v2522', 'v2523', 'v0717'], axis = 1)    

# Show odds values as well
pd.set_option('display.max_rows', None)    # Show all rows
log_reg = sm.ZeroInflatedNegativeBinomialP(output, inputs).fit(method='nm') #bfgs or nm
