...
variables = df[['v2500',                            # Education
                ...
                'v2520',                            # alcohol violations
                'v2521',                            # count alcohol violations
...
# alcohol rule violation count
df_statsbinary['v2520'] = df_statsbinary['v2520'].apply(lambda v: 1 if v == 'Yes' else 0)
df_statsbinary['v2521'] = np.where(
    df_statsbinary['v2520'] == 0,  
    0,                       
    df_statsbinary['v2521']      
)
print(df_statsbinary['v2521'].value_counts())
df_statsbinary['v2521'] = df_statsbinary['v2521'].apply(lambda v: int(v) if not \
                                            (v == 'Blank' or
                                            v == 'Don\'t know' or
                                            v == 'Refused')
                                            else None)
df_statsbinary = df_statsbinary.dropna(subset=['v2521'])
...
output = df_statsbinary['v2521']
inputs = df_statsbinary.drop(['v2520', 'v2521', 'v0717'], axis = 1)    
