...
variables = df[['v2500',                            # Education
                ...
                'v2538',                            # out of place violations
                'v2539',                            # count out of place violations
...
# out of place rule violation count
df_statsbinary['v2538'] = df_statsbinary['v2538'].apply(lambda v: 1 if v == 'Yes' else 0)
df_statsbinary['v2539'] = np.where(
    df_statsbinary['v2538'] == 0,  
    0,                       
    df_statsbinary['v2539']      
)
print(df_statsbinary['v2539'].value_counts())
df_statsbinary['v2539'] = df_statsbinary['v2539'].apply(lambda v: int(v) if not \
                                            (v == 'Blank' or
                                            v == 'Don\'t know' or
                                            v == 'Refused')
                                            else None)
df_statsbinary = df_statsbinary.dropna(subset=['v2539'])
...
output = df_statsbinary['v2539']
inputs = df_statsbinary.drop(['v2538', 'v2539', 'v0717'], axis = 1)    
