...
variables = df[['v2500',                            # Education
                ...
                'v2530',                            # physical assault staff violations
                'v2531',                            # count physical assault staff violations
...
# physical assault staff rule violation count
df_statsbinary['v2530'] = df_statsbinary['v2530'].apply(lambda v: 1 if v == 'Yes' else 0)
df_statsbinary['v2531'] = np.where(
    df_statsbinary['v2530'] == 0,  
    0,                       
    df_statsbinary['v2531']      
)
print(df_statsbinary['v2531'].value_counts())
df_statsbinary['v2531'] = df_statsbinary['v2531'].apply(lambda v: int(v) if not \
                                            (v == 'Blank' or
                                            v == 'Don\'t know' or
                                            v == 'Refused')
                                            else None)
df_statsbinary = df_statsbinary.dropna(subset=['v2531'])
...
output = df_statsbinary['v2531']
inputs = df_statsbinary.drop(['v2530', 'v2531', 'v0717'], axis = 1)    
