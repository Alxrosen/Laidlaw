...
#removes blank and dk from df
df_statsbinary = variables[
    ((variables['v2549'] != 'Blank') &
     (variables['v2549'] != 'Don\'t know'))]
...
# convert to major vs. minor rule violations
'''
NOTE
This is my own classification...
'''
def create_severity_binary(category):
    major_violations = [
    ...
    return 1 if category in major_violations else 0

# Apply the function to create binary outcome
output = df_statsbinary['v2549']
output_binary = output.apply(create_severity_binary)
inputs = df_statsbinary.drop(['v2549', 'v0717'], axis = 1)

# Show odds values as well
pd.set_option('display.max_rows', None)    # Show all rows
print(inputs.nunique())             #Debuge
print(inputs.T.duplicated())        #Debug
log_reg = sm.Logit(output_binary, inputs).fit()
