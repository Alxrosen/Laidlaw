...
variables = df[['v2500',                            # Education
                ...
                'v2544',                            # other minor  violations
                'v2545',                            # count other minor  violations
...
# other minor  rule violation count
df_statsbinary['v2544'] = df_statsbinary['v2544'].apply(lambda v: 1 if v == 'Yes' else 0)
df_statsbinary['v2545'] = np.where(
    df_statsbinary['v2544'] == 0,  
    0,                       
    df_statsbinary['v2545']      
)
print(df_statsbinary['v2545'].value_counts())
df_statsbinary['v2545'] = df_statsbinary['v2545'].apply(lambda v: int(v) if not \
                                            (v == 'Blank' or
                                            v == 'Don\'t know' or
                                            v == 'Refused')
                                            else None)
df_statsbinary = df_statsbinary.dropna(subset=['v2545'])
...
output = df_statsbinary['v2545']
inputs = df_statsbinary.drop(['v2544', 'v2545', 'v0717'], axis = 1)    
