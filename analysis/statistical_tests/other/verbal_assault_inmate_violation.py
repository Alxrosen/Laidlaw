...
variables = df[['v2500',                            # Education
                ...
                'v2518',                            # drug violations
                'v2519',                            # count drug violations
...
# drug rule violation count
df_statsbinary['v2518'] = df_statsbinary['v2518'].apply(lambda v: 1 if v == 'Yes' else 0)
df_statsbinary['v2519'] = np.where(
    df_statsbinary['v2518'] == 0,  
    0,                       
    df_statsbinary['v2519']      
)
print(df_statsbinary['v2519'].value_counts())
df_statsbinary['v2519'] = df_statsbinary['v2519'].apply(lambda v: int(v) if not \
                                            (v == 'Blank' or
                                            v == 'Don\'t know' or
                                            v == 'Refused')
                                            else None)
df_statsbinary = df_statsbinary.dropna(subset=['v2519'])
...
output = df_statsbinary['v2519']
inputs = df_statsbinary.drop(['v2518', 'v2519', 'v0717'], axis = 1)    
